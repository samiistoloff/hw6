...
class Student():
    ...
    def write_programs(self, prm = 1):
        self.num_programs = self.num_programs + prm
...
def personal_map(f_obj, f_lst):
    return [f_obj(f) for f in f_lst]
...
names_and_productivities = [(t[0],t[1]) for t in zip(names, prod_list)]
...
names_with_not_too_much_seniority = [t[0] for t in student_tups_list if len(t[0]) > t[1]]
